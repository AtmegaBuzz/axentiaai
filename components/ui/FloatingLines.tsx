...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Use line brightness as alpha so the black background is transparent
  // and the section's light gradient shows through
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      // Check if pointer is within the section bounds
      const inBounds =
    ...
    if (interactive) {
      // Listen on window so events aren't blocked by overlapping z-index elements
    ...
    // Pause render loop when off-screen to save GPU/CPU
