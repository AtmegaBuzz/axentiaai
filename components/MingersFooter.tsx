...
export function MingersFooter() {
    return (
        <footer className="bg-white font-sans text-slate-900 border-t border-slate-100">
            ...
            <div className="container mx-auto px-4 md:px-8 xl:px-12 py-16">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8">
                    ...
                    {Object.entries(footerLinks).map(([title, links]) => (
                        <div key={title}>
                            <h4 className="font-bold text-lg mb-6 text-slate-900">{title}</h4>
                            <ul className="space-y-4">
                                {links.map((link) => (
                                    <li key={link.name}>
                                        <Link href={link.href} className="text-slate-600 hover:text-brand-600 font-medium transition-colors">
                                            {link.name}
                                        </Link>
                                    </li>
                                ))}
                    ...
                    <div className="lg:col-span-2 bg-slate-50 p-8 rounded-3xl border border-slate-100">
                        <h4 className="font-bold text-2xl mb-3 text-slate-900">Stay Updated</h4>
                        ...
                        <form className="flex flex-col sm:flex-row gap-3">
                            <input
                                ...
                                className="flex-1 px-5 py-3 rounded-full border border-slate-200 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all font-medium"
                                required
                            />
                            <button
                                type="submit"
                        ...
                        <p className="text-xs text-slate-500 mt-4 leading-relaxed">
                            By subscribing, you agree to our <Link href="#" className="underline hover:text-slate-800">Privacy Policy.</Link>
                        </p>
                    </div>
            ...
            {/* ── CONTACT ROW ── */}
            <div className="container mx-auto px-4 md:px-8 xl:px-12 pb-8">
                <div className="border-t border-slate-200 pt-8">
                    <div className="flex flex-wrap gap-8 text-sm text-slate-600">
                        <a href="tel:+919355181110" className="flex items-center gap-2 hover:text-brand-600 transition-colors font-medium">
                            <Phone className="w-4 h-4 text-brand-500" /> +91-9355181110
                        </a>
                        <a href="mailto:info@axentiaai.in" className="flex items-center gap-2 hover:text-brand-600 transition-colors font-medium">
            ...
            {/* ── BOTTOM BAR ── */}
            <div className="container mx-auto px-4 md:px-8 xl:px-12 pb-12">

                {/* Logo & Socials */}
                <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-6">
                    <Link href="/" className="flex items-center">
                        <Image src="/logo.png" alt="Axentia.AI" width={320} height={96} className="h-24 w-auto" />
                    </Link>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-sm font-medium text-slate-500">
                    <div className="flex flex-wrap items-center justify-center gap-4">
                        ...
                        <Link href="#" className="hover:text-slate-800 transition-colors">Terms & Conditions</Link>
                        <span className="text-slate-300">|</span>
